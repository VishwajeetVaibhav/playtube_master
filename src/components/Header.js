...
const Header = () => {
  ...
  return (
    <div className="sticky top-0 z-20 flex flex-row items-center justify-between px-4 bg-white shadow-md h-14 md:px-5 dark:bg-black">
      {loading && <Loader />}
      ...
      <div className="flex items-center h-5">
        {pageName !== "video" && (
          ...
          >
            {mobileMenu ? (
              <CgClose className="text-xl text-black dark:text-white" />
            ) : (
              <SlMenu className="text-xl text-black dark:text-white" />
            )}
          </div>
        )}
      ...
      <div className="flex items-center">
        <div className="hidden md:flex">
          ...
          >
            {theme === "dark" ? (
              <MdOutlineLightMode className="text-xl text-white cursor-pointer" />
            ) : (
        ...
        <div className="flex w-8 h-8 overflow-hidden rounded-full md:ml-4">
          <Link to={"https://github.com/VishwajeetVaibhav"}>
            <img
              src="https://avatars.githubusercontent.com/u/108585532?v=4"
              alt="Vishwajeet Vaibhav"
            />
          </Link>
        </div>
      </div>
